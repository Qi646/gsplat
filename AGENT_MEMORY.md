...
- Preset scene loads now use hardcoded per-preset calibration metadata in both renderer adapters: `Luigi` keeps identity rotation, while `Garden`, `Stump`, and `Truck` apply a 180-degree X flip plus preset-specific default camera views. Manual URL loads still preserve source orientation until the user rotates the scene explicitly.
...
- Walk mode now uses an explicit `inactive` -> `armed` -> `active` lifecycle with quaternion mouse look: clicking the button arms it, the next click inside the viewer requests pointer lock, mouse look preserves arbitrary roll, `W/S` move along the camera look direction, `A/D` strafe, `Q/E` follow camera-local up, and exiting walk mode re-syncs the shared camera controls to the current view.
...
- Robust framing now compares both wide `1%-99%` sampled bounds and tighter `5%-95%` sampled bounds, then uses the tighter framing candidate when the wide/tight bounding-sphere radius ratio exceeds `1.4`; this materially reduces initial over-zoomed views on `Stump` and `Truck`.
- The root UI now includes shared manual `Orientation` controls for any loaded scene. Orientation changes are treated as scene state, are disabled during preview/export/walk mode, clear existing keyframes after confirmation, and refresh the reset-view snapshot from the new framing.
...
- After the preset-calibration + roll-aware walk update on 2026-03-10, root `npm test` and `npm run build` passed again; both renderer adapters now share preset upright/default-view metadata, manual scene rotation, tighter framing fallback, roll-preserving walk mode, and camera-path v2 scene-rotation serialization.
...
- Treat preset upright/default views as hardcoded calibration metadata, not inferred asset metadata; user-loaded scenes should stay source-native until manually rotated.
...
- Treat scene rotation as scene state instead of camera state: applying a manual scene rotation should invalidate existing keyframes and refresh the reset-view snapshot from a fresh generic frame.
...
- 2026-03-10: Added shared preset upright/default-view calibration, manual scene-orientation controls, a tighter framing fallback for outlier-heavy scenes, roll-preserving walk mode, and camera-path v2 `sceneRotation` support across both renderer adapters; revalidated with passing root `npm test` / `npm run build`.
