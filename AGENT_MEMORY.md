...
- Walk mode now uses an explicit `inactive` -> `armed` -> `active` lifecycle with quaternion mouse look: pressing `2` or clicking `Walk Mode` requests pointer lock immediately, pressing `1` returns to Inspect mode, the top bar shows the current navigation mode, mouse look preserves arbitrary roll, `W/S` move along the camera look direction, `A/D` strafe, `Q/E` follow camera-local up, and exiting walk mode re-syncs the shared camera controls to the current view.
