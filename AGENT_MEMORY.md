...
- The root app now supports test-only startup query params via `client/src/lib/runtimeQuery.ts`: `?e2e=1` enables `window.__GSPLAT_DEBUG__`, `?viewerMode=compat` forces compatibility worker settings, and `?scene=...` can auto-load a fixture URL.
