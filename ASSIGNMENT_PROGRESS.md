...
- Latest validation on 2026-03-12: `npm test --workspace=client -- agenticPathFeedback agenticPath SceneViewer SparkSceneViewer ExportManager`, `npm run build --workspace=client`, `npm test --workspace=server -- pathGeneration app`, `npm run build --workspace=server`, `npm test`, and `npm run build` passed.
...
| 2. Scene navigation controls | Implemented | Orbit / pan / zoom are available through the shared camera controls, and the user-facing `Fly Mode` provides fly-style WASD + mouse-look navigation with pointer lock. Press `2` or click `Fly [2]` to capture the cursor immediately, press `1` to return to Inspect mode, and the top bar now shows the current navigation mode. `Z/C` roll the camera around its current view axis in both Inspect and active Fly mode, mouse look preserves arbitrary roll, and `Q/E` follow the camera-local up axis so fly mode stays coherent after inverted views. |
| 3. Camera path recording | Implemented | Users can add keyframes, delete them, reorder them, scrub the path, preview playback, and inspect a toggleable in-view path/frustum overlay. Clicking a keyframe in the list jumps the camera there and makes the next preview start from that selected keyframe instead of always restarting at the beginning. Playback uses Catmull-Rom position interpolation, quaternion slerp, FOV interpolation, and fixed smoothstep ease-in/out timing, while the shared frustum gizmos now clamp their on-screen size when zoomed in and around tightly clustered keyframes to reduce overlap. Saved path JSON now writes the original v1 schema while still accepting older v2 imports that may contain deprecated `sceneRotation`. The path panel now supports draft-based `multistep-v2` generation for both one continuous subject-centric move and one continuous route-following traverse. It captures the current view plus 6 nearby scout views that stay centered on the user-framed camera ray with a wider angular spread, can add one bounded 4-view rescan round when grounding is weak, and chooses that rescue round around either the original view ray, a provisional subject anchor, or a provisional route midpoint depending on the prompt mode and evidence. It calls separate `/api/path/ground`, `/api/path/compose`, and `/api/path/verify` planner steps, synthesizes deterministic local draft keyframes, keeps overly close starts and downward motion above the scene-floor / subject-safety thresholds for subject-centric drafts, grounds route-following drafts from sampled scene points into one ordered 3D route polyline, and then drives a bounded `/api/path/verify` pass with exact draft samples plus a few risk-focused off-path probe captures so longer drafts can gather more evidence without turning into unbounded exploration. The panel also exposes explicit `Draft Length` and `Ending Hold` controls so timing and hold behavior are not left entirely to free-form prompt wording. Drafts can be previewed, discarded, or regenerated; multi-subject and ambiguous prompts still fail early with inline feedback, and the route-following slice still assumes one unbranched route that is already partly visible from the starting view. The default planner model is now `gpt-5-mini`, and the server-side planner request still auto-negotiates common OpenAI model/request compatibility differences when `OPENAI_MODEL` or `OPENAI_BASE_URL` is overridden, including token-budget field swaps, GPT-5's default-only temperature rule, a minimal GPT-5 reasoning setting plus larger completion budget to leave room for final JSON, and a no-`response_format` fallback if JSON mode is rejected. The planner response parser now also accepts structured text payloads, loose enum synonyms, numeric `verticalBias` hints, route observations, `traverse` segments, and route verify metadata aliases instead of assuming one rigid completion-content shape. Generation still shows staged capture/planning progress, locks viewer interaction while the camera is being driven for captures, offers a user cancel action in the blocking UI, fails closed with a client timeout so controls are restored if the flow stalls, and surfaces failures in a dedicated inline callout with retry guidance. When the server is started without `OPENAI_API_KEY`, those prompt controls stay disabled instead of breaking the rest of the app. Caveat: manual keyframe timing is still not directly editable beyond reorder + scrub, even though the draft generator now accepts timing goals. |
...
- Roll-preserving quaternion fly mode plus backward-compatible camera-path import for older v2 files with deprecated `sceneRotation`.
