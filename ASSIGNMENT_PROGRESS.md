...
| 3. Camera path recording | Implemented | Users can add keyframes, delete them, reorder them, scrub the path, preview playback, and inspect a toggleable in-view path/frustum overlay. Clicking a keyframe in the list jumps the camera there and makes the next preview start from that selected keyframe instead of always restarting at the beginning. Playback uses Catmull-Rom position interpolation, quaternion slerp, FOV interpolation, and fixed smoothstep ease-in/out timing, while the shared frustum gizmos now clamp their on-screen size when zoomed in and around tightly clustered keyframes to reduce overlap. Saved path JSON now writes the original v1 schema while still accepting older v2 imports that may contain deprecated `sceneRotation`. The path panel now exposes a `Draft Strategy` selector. `Planner Draft` keeps the existing `multistep-v2` subject-centric / route-following flow with nearby scout captures, bounded rescans, deterministic validation, and planner verification. `Stepwise Agent (Experimental)` instead captures the live view each step, sends the server a bounded list of legal candidate actions with runtime-authored predicted-outcome summaries, and asks the model to infer a local next-step intent before choosing one `move` / `rotate` / `capture-image` / `create-keyframe` action while still keeping the live path unchanged until `Apply Draft`; its deterministic move primitives now use camera-local yaw/up/forward/right axes so rolled or upside-down starting poses stay coherent with Fly mode, and pure in-place rotations remain allowed for small-subject reframing even when the robust scene bounds are tighter than the current camera standoff. Both modes still fail closed with inline feedback, viewer locking during capture-driven generation, and explicit cancel / timeout recovery. Caveat: manual keyframe timing is still not directly editable beyond reorder + scrub, even though both prompt flows accept timing goals. |
