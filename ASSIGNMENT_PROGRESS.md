...
| 3. Camera path recording | Implemented | Users can add keyframes, delete them, reorder them, scrub the path, preview playback, and inspect a toggleable in-view path/frustum overlay. Clicking a keyframe in the list jumps the camera there and makes the next preview start from that selected keyframe instead of always restarting at the beginning. Playback uses Catmull-Rom position interpolation, quaternion slerp, FOV interpolation, and fixed smoothstep ease-in/out timing, while the shared frustum gizmos now clamp their on-screen size when zoomed in and around tightly clustered keyframes to reduce overlap. Saved path JSON now writes the original v1 schema while still accepting older v2 imports that may contain deprecated `sceneRotation`. The path panel also supports prompt-driven orbit generation that appends ordinary keyframes after image-conditioned server planning; the scout captures now stay near the live camera pose in a small multi-axis spherical pattern, and both those captures and the generated orbit keyframes derive their orbit basis from the live camera basis instead of assuming world Y-up, so arbitrary scene orientations preserve the initial framing and roll. The server-side planner request now also auto-negotiates common OpenAI model/request compatibility differences when `OPENAI_MODEL` or `OPENAI_BASE_URL` is overridden, including token-budget field swaps, GPT-5's default-only temperature rule, a minimal GPT-5 reasoning setting plus larger completion budget to leave room for final JSON, and a no-`response_format` fallback if JSON mode is rejected. The planner response parser now also accepts structured text payloads instead of assuming one fixed completion-content shape. Generation still shows staged capture/planning progress, locks viewer interaction while the camera is being driven for scout captures, offers a user cancel action in the blocking UI, fails closed with a client timeout so controls are restored if the flow stalls, and now surfaces failures in a dedicated inline error callout with retry guidance. When the server is started without `OPENAI_API_KEY`, those prompt controls stay disabled instead of breaking the rest of the app. Caveat: keyframe timing is not directly editable beyond reorder + scrub. |
