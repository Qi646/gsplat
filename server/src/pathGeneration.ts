...
function buildComposeSystemPrompt(): string {
  return [
    ...
    'Arc.direction, when present, must be exactly "clockwise" or "counterclockwise".',
    ...
    'Only include fields that belong to that segment type. Do not borrow travelDirection, lateralBias, or sweepDegrees across incompatible segment types.',
    'Do not use descriptive substitutes like "frontal", "forward-facing", "toward-subject", "push-in", "pull-back", "rise", "drop", or "middle" in enum fields; use the exact allowed literals.',
    'Canonical valid segment examples: {"segmentType":"dolly","durationSeconds":4,"lookMode":"look-at-subject","travelDirection":"in"}, {"segmentType":"traverse","durationSeconds":6,"lookMode":"look-forward","lateralBias":"center","verticalBias":"mid"}, {"segmentType":"arc","durationSeconds":5,"lookMode":"look-at-subject","direction":"clockwise","sweepDegrees":120}.',
    'Canonical invalid segment examples: {"segmentType":"dolly","durationSeconds":4,"lookMode":"frontal","travelDirection":"push-in"}, {"segmentType":"pedestal","durationSeconds":4,"lookMode":"look-at-subject","travelDirection":"rise"}, {"segmentType":"traverse","durationSeconds":6,"lookMode":"look-forward","lateralBias":"middle"}.',
...
function buildVerifySystemPrompt(): string {
  return [
    ...
    'issues must be an array of non-empty strings. If approved is false, issues must not be empty.',
    ...
    'probeReason values in the input use exact enums such as "overview", "floor-clearance", "subject-framing", "subject-distance", "segment-transition", "hold-read", and "long-path-lookahead". Treat them as labels, but do not rewrite them in the response because the response only returns approved/issues/warning.',
...
function buildStepSystemPrompt(): string {
  return [
    ...
    'Classify the prompt into exactly one pathMode: "subject-centric", "route-following", "multi-subject", or "ambiguous".',
    'pathMode must be exactly one of those strings; do not use synonyms.',
    ...
    'Each candidateAssessment.action must use the same exact chosenAction enum contract described below.',
    ...
    'Use pathMode only for support classification and to interpret explicit route-vs-subject evidence already present in the frame or candidate summaries, not as a shortcut from label to primitive.',
    'Set complete=true only when the existing draft keyframes already represent the requested move well enough that another action is unnecessary.',
