...
- The Camera Path panel now includes draft-based subject-centric generation. It captures the current view plus 6 nearby scout views that stay centered on the original view ray while using a moderate angular spread, can add a tighter 4-view targeted rescan when grounding is weak, and also spends that rescue round on the original view ray if only the current capture sees the subject at first. Those captures go through `/api/path/ground` and `/api/path/compose`, then the client builds and validates the draft locally before showing `Preview Draft`, `Apply Draft`, `Discard Draft`, and `Regenerate`. If the live camera starts too close to the grounded subject, the draft builder now first nudges the draft start pose back to a safe distance instead of failing validation immediately. While a draft exists, the overlay shows the draft instead of the live path, and the live keyframes remain unchanged until `Apply Draft`.
...
- To use the prompt flow, load a scene such as `Truck`, enter a request like `Create one continuous cinematic arc around this truck, keep the camera focused on it, then hold for a beat.`, click `Generate Draft`, then `Preview Draft` or `Apply Draft`.
