...
- `client/src/path/PathInterpolator.ts`, `client/src/path/KeyframeManager.ts`, `client/src/path/cameraPath.ts`, and `client/src/path/agenticPath.ts` own manual keyframe capture, prompt-driven orbit generation, interpolation, preview playback, and path JSON serialization. Agentic orbit generation now starts from the live camera view, runs an iterative planner loop, and executes planner-requested follow-up captures relative to existing views instead of relying on a fixed scout batch. Those follow-up captures and the generated orbit keyframes both preserve the live camera basis instead of assuming every scene is Y-up. Saved paths still write camera-path JSON v1 and the importer still accepts older v2 files that include deprecated `sceneRotation`.
...
- The Camera Path panel now includes prompt-driven orbit generation. It captures the current view, sends it to the server vision planner, and then runs an iterative loop where the planner can request additional targeted follow-up captures relative to existing views before returning a final orbit spec plus subject localizations. Those follow-up captures and generated orbit keyframes preserve the live camera's derived orbit axis and roll instead of forcing a world-Y frame, so non-Y-up scenes keep the initial framing and no longer flip upside down during generation. If generation still fails, the panel shows a dedicated inline error card with the raw planner failure plus retry guidance instead of only updating the footer status note. While the flow runs, the UI shows capture/planning progress, blocks viewer interaction so the planner-driven camera moves are explicit, offers a visible `Cancel` action, and fails closed with a client timeout that restores controls if the flow stalls.
...
- Agentic path generation is orbit-only in v1. `OPENAI_BASE_URL` and `OPENAI_MODEL` are optional overrides for OpenAI-compatible endpoints when `OPENAI_API_KEY` is configured. The planner server now auto-negotiates common request-shape differences across models and compatible endpoints, including `max_completion_tokens` vs `max_tokens`, GPT-5's default-only temperature handling, and a fallback path without `response_format` if JSON mode is rejected.
