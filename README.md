...
- The Camera Path panel now includes prompt-driven orbit generation. It captures the current view plus four scout views, sends them to the server vision planner, triangulates a subject anchor locally, and appends ordinary keyframes back into the existing path.
...
- Agentic path generation is orbit-only in v1 and requires `OPENAI_API_KEY` on the server. `OPENAI_BASE_URL` and `OPENAI_MODEL` are optional overrides for OpenAI-compatible endpoints.
