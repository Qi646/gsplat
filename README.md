...
- `client/src/controls/WalkControls.ts` now runs walk mode as an `inactive` -> `armed` -> `active` state machine using quaternion camera updates: clicking `Walk Mode` arms it, the next click in the viewer requests pointer lock, mouse look preserves arbitrary camera roll, `W/S` move forward/back along the camera look direction, `A/D` strafe, `Q/E` follow the camera-local up axis, and leaving walk mode hands control back to the default camera controls without changing the live camera pose.
- `client/src/lib/sceneFormat.ts`, `client/src/lib/scenePresets.ts`, and `client/src/main.ts` isolate URL format detection, hardcoded preset calibration metadata, and the app-level manual scene-orientation controls that are shared across both renderer adapters.
...
- Initial framing and `Frame Scene` now use robust sampled bounds that ignore low-alpha outliers before falling back to the raw mesh bounding box. When the usual `1%-99%` framing sphere is much larger than a tighter `5%-95%` candidate, the viewer automatically uses the tighter fit to avoid obviously zoomed-out starts on presets like `Stump` and `Truck`.
...
- The four built-in presets now load through hardcoded calibration metadata: each preset can apply a scene-level upright rotation plus a preset-specific default camera view on first load.
- Manual URL loads still preserve their source orientation by default. The `Orientation` controls let users apply `Yaw`, `Pitch`, and `Roll` scene rotations in `90` degree steps, and `Reset Orientation` restores the preset baseline rotation or identity rotation for uploads.
...
- Changing scene orientation is treated as scene state rather than camera state. Manual scene-rotation changes stop preview, are blocked during export and active walk mode, clear any recorded keyframes after confirmation, run a fresh generic `Frame Scene`, and refresh the `Reset View` snapshot from that new pose.
...
- Walk-mode mouse look is roll-aware, so entering walk mode from an inverted or manually re-oriented scene no longer forces yaw/pitch behavior back through a world-up Euler decomposition.
- Path import and path editing controls remain disabled until a scene has loaded successfully.
- Saved camera paths now include the current scene rotation when needed. The importer accepts both legacy v1 JSON and the current v2 schema, and applies scene rotation before restoring keyframes.
