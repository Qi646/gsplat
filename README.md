...
- The Camera Path panel now includes draft-based subject-centric generation. It captures the current view plus 6 nearby scout views that stay close to the original framing, can add a tighter 4-view targeted rescan when grounding is weak, and also spends that rescue round on the original view ray if only the current capture sees the subject at first. Those captures go through `/api/path/ground` and `/api/path/compose`, then the client builds and validates the draft locally before showing `Preview Draft`, `Apply Draft`, `Discard Draft`, and `Regenerate`. While a draft exists, the overlay shows the draft instead of the live path, and the live keyframes remain unchanged until `Apply Draft`.
